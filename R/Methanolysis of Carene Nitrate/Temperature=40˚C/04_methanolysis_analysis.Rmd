...
```{r, message=FALSE}
methanolysis_data <- read_csv("/Users/jamesvesto/R-Studio Files/Projects/terpene-kinetics/data/AH-01-15zg2d.csv")
...
mplt <- ggplot(methanolysis, aes(time_s,integral_mM,color=assignment))+
  ...
  labs(x="Time (s)", 
       y="Concentration (mM)", 
       title="Trial #4 Carene Nitrate Methanolysis: AH-01-15zg2d")+
  theme_bw()+ 
  annotate("text", x=4000,y=7.5, label=expression('[A]'[t]*'=[A]'[0]*'e'^{'-1.9E-4'*t}))+
  ...
  annotation_raster(raster,6000, 8500,5.0, 7.59)+
...
```{r}
save_plot("/Users/jamesvesto/R-Studio Files/Projects/terpene-kinetics/images/plots/AH-01-15zg2d.jpg", mplt)
rm(list=ls()) 
```